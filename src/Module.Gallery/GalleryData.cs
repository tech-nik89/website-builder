...
namespace WebsiteBuilder.Modules.Gallery {
    class GalleryData {
        ...
        public static String Serialize(GalleryData data) {
            XDocument doc = new XDocument(new XElement(RootTag,
                ...
                data.Files.Select(x => new XElement(FileTag, x))
            ));

            return doc.ToString();
        }

        public static GalleryData Deserialize(String input) {
            ...
            try {
                ...
                data.Files = root.Elements(FileTag).Select(x => x.Value).ToList();
